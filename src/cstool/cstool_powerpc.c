	...
	for (i = 0; i < ppc->op_count; i++) {
		...
		switch((int)op->type) {
			...
			case PPC_OP_MEM:
				...
				break;
			case PPC_OP_CRX:
				printf_to_string("\t\toperands[%u].type: CRX\n", i);
				printf_to_string("\t\t\toperands[%u].crx.scale: %d\n", i, op->crx.scale);
				printf_to_string("\t\t\toperands[%u].crx.reg: %s\n", i, cs_reg_name(handle, op->crx.reg));
				printf_to_string("\t\t\toperands[%u].crx.cond: %s\n", i, get_bc_name(op->crx.cond));
				break;
		}
	}

	if (ppc->bc != 0)
		printf_to_string("\tBranch code: %u\n", ppc->bc);

	if (ppc->bh != 0)
		printf_to_string("\tBranch hint: %u\n", ppc->bh);

	if (ppc->update_cr0)
		printf_to_string("\tUpdate-CR0: True\n");
}
