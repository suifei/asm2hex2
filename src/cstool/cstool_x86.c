...
void print_insn_detail_x86(csh ud, cs_mode mode, cs_insn *ins)
{
	...
	printf_to_string("\trex: 0x%x\n", x86->rex);
	printf_to_string("\taddr_size: %u\n", x86->addr_size);
	printf_to_string("\tmodrm: 0x%x\n", x86->modrm);
	printf_to_string("\tdisp: 0x%" PRIx64 "\n", x86->disp);

	// SIB is not available in 16-bit mode
	if ((mode & CS_MODE_16) == 0) {
		printf_to_string("\tsib: 0x%x\n", x86->sib);
		if (x86->sib_base != X86_REG_INVALID)
			printf_to_string("\t\tsib_base: %s\n", cs_reg_name(ud, x86->sib_base));
		if (x86->sib_index != X86_REG_INVALID)
			printf_to_string("\t\tsib_index: %s\n", cs_reg_name(ud, x86->sib_index));
		if (x86->sib_scale != 0)
			printf_to_string("\t\tsib_scale: %d\n", x86->sib_scale);
	}

	// XOP code condition
	if (x86->xop_cc != X86_XOP_CC_INVALID) {
		printf_to_string("\txop_cc: %u\n", x86->xop_cc);
	}

	// SSE code condition
	if (x86->sse_cc != X86_SSE_CC_INVALID) {
		printf_to_string("\tsse_cc: %u\n", x86->sse_cc);
	}

	// AVX code condition
	if (x86->avx_cc != X86_AVX_CC_INVALID) {
		printf_to_string("\tavx_cc: %u\n", x86->avx_cc);
	}

	// AVX Suppress All Exception
	if (x86->avx_sae) {
		printf_to_string("\tavx_sae: %u\n", x86->avx_sae);
	}

	// AVX Rounding Mode
	if (x86->avx_rm != X86_AVX_RM_INVALID) {
		printf_to_string("\tavx_rm: %u\n", x86->avx_rm);
	}

	// Print out all immediate operands
	count = cs_op_count(ud, ins, X86_OP_IMM);
	if (count > 0) {
		printf_to_string("\timm_count: %u\n", count);
		for (i = 1; i < count + 1; i++) {
			int index = cs_op_index(ud, ins, X86_OP_IMM, i);
			printf_to_string("\t\timms[%u]: 0x%" PRIx64 "\n", i, x86->operands[index].imm);
		}
	}
	...
	for (i = 0; i < x86->op_count; i++) {
		...
		switch((int)op->type) {
			...
			case X86_OP_MEM:
				...
				if (op->mem.segment != X86_REG_INVALID)
					printf_to_string("\t\t\toperands[%u].mem.segment: REG = %s\n", i, cs_reg_name(ud, op->mem.segment));
				...
				if (op->mem.scale != 1)
					printf_to_string("\t\t\toperands[%u].mem.scale: %u\n", i, op->mem.scale);
				if (op->mem.disp != 0)
					printf_to_string("\t\t\toperands[%u].mem.disp: 0x%" PRIx64 "\n", i, op->mem.disp);
				break;
			default:
				break;
		}
		...
		if (op->avx_bcast != X86_AVX_BCAST_INVALID)
			printf_to_string("\t\toperands[%u].avx_bcast: %u\n", i, op->avx_bcast);

		// AVX zero opmask {z}
		if (op->avx_zero_opmask != false)
			printf_to_string("\t\toperands[%u].avx_zero_opmask: TRUE\n", i);

		printf_to_string("\t\toperands[%u].size: %u\n", i, op->size);
	...
	if (!cs_regs_access(ud, ins,
		...
		if (regs_read_count) {
			...
			for(i = 0; i < regs_read_count; i++) {
				printf_to_string(" %s", cs_reg_name(ud, regs_read[i]));
			}
			printf_to_string("\n");
		}
		...
		if (regs_write_count) {
			...
			for(i = 0; i < regs_write_count; i++) {
				printf_to_string(" %s", cs_reg_name(ud, regs_write[i]));
			}
			printf_to_string("\n");
		}
	}
	...
	if (x86->eflags || x86->fpu_flags) {
		for(i = 0; i < ins->detail->groups_count; i++) {
			if (ins->detail->groups[i] == X86_GRP_FPU) {
				printf_to_string("\tFPU_FLAGS:");
				for(i = 0; i <= 63; i++)
					if (x86->fpu_flags & ((uint64_t)1 << i)) {
						printf_to_string(" %s", get_fpu_flag_name((uint64_t)1 << i));
					}
				printf_to_string("\n");
				break;
			}
		...
		if (i == ins->detail->groups_count) {
			printf_to_string("\tEFLAGS:");
			for(i = 0; i <= 63; i++)
				if (x86->eflags & ((uint64_t)1 << i)) {
					printf_to_string(" %s", get_eflag_name((uint64_t)1 << i));
				}
			printf_to_string("\n");
		}
	}
