	...
	for (i = 0; i < arm64->op_count; i++) {
		...
		switch(op->type) {
			...
			case ARM64_OP_MEM:
				...
				break;
			case ARM64_OP_CIMM:
				printf_to_string("\t\toperands[%u].type: C-IMM = %u\n", i, (int)op->imm);
				break;
			case ARM64_OP_REG_MRS:
				printf_to_string("\t\toperands[%u].type: REG_MRS = 0x%x\n", i, op->reg);
				break;
			case ARM64_OP_REG_MSR:
				printf_to_string("\t\toperands[%u].type: REG_MSR = 0x%x\n", i, op->reg);
				break;
			case ARM64_OP_PSTATE:
				printf_to_string("\t\toperands[%u].type: PSTATE = 0x%x\n", i, op->pstate);
				break;
			case ARM64_OP_SYS:
				printf_to_string("\t\toperands[%u].type: SYS = 0x%x\n", i, op->sys);
				break;
			case ARM64_OP_PREFETCH:
				printf_to_string("\t\toperands[%u].type: PREFETCH = 0x%x\n", i, op->prefetch);
				break;
			case ARM64_OP_BARRIER:
				printf_to_string("\t\toperands[%u].type: BARRIER = 0x%x\n", i, op->barrier);
				break;
			case ARM64_OP_SVCR:
				printf_to_string("\t\toperands[%u].type: SYS = 0x%x\n", i, op->sys);
				if(op->svcr == ARM64_SVCR_SVCRSM)
					printf_to_string("\t\t\toperands[%u].svcr: BIT = SM\n", i);
				if(op->svcr == ARM64_SVCR_SVCRZA)
					printf_to_string("\t\t\toperands[%u].svcr: BIT = ZA\n", i);
				if(op->svcr == ARM64_SVCR_SVCRSMZA)
					printf_to_string("\t\t\toperands[%u].svcr: BIT = SM & ZA\n", i);
				break;
			case ARM64_OP_SME_INDEX:
				printf_to_string("\t\toperands[%u].type: REG = %s\n", i, cs_reg_name(handle, op->sme_index.reg));
				if (op->sme_index.base != ARM64_REG_INVALID)
					printf_to_string("\t\t\toperands[%u].index.base: REG = %s\n", i, cs_reg_name(handle, op->sme_index.base));
				if (op->sme_index.disp != 0)
					printf_to_string("\t\t\toperands[%u].index.disp: 0x%x\n", i, op->sme_index.disp);
				break;
		}
		...
		if (op->shift.type != ARM64_SFT_INVALID &&
			op->shift.value)
			printf_to_string("\t\t\tShift: type = %u, value = %u\n",
				   op->shift.type, op->shift.value);

		if (op->ext != ARM64_EXT_INVALID)
			printf_to_string("\t\t\tExt: %u\n", op->ext);

		if (op->vas != ARM64_VAS_INVALID)
			printf_to_string("\t\t\tVector Arrangement Specifier: 0x%x\n", op->vas);

		if (op->vector_index != -1)
			printf_to_string("\t\t\tVector Index: %u\n", op->vector_index);
	}
