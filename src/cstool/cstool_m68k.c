...
static void print_read_write_regs(cs_detail* detail, csh handle)
{
	...
	for (i = 0; i < detail->regs_read_count; ++i) {
		...
		printf_to_string("\treading from reg: %s\n", reg_name);
	}

	for (i = 0; i < detail->regs_write_count; ++i) {
		uint16_t reg_id = detail->regs_write[i];
		const char* reg_name = cs_reg_name(handle, reg_id);
		printf_to_string("\twriting to reg:   %s\n", reg_name);
	}
}
...
void print_insn_detail_m68k(csh handle, cs_insn *ins)
{
	...
	printf_to_string("\tgroups_count: %u\n", detail->groups_count);
		...
		switch((int)op->type) {
			...
			case M68K_OP_MEM:
				...
				if (op->mem.index_reg != M68K_REG_INVALID) {
					...
					printf_to_string("\t\t\toperands[%u].mem.index: size = %c\n",
				...
				printf_to_string("\t\taddress mode: %s\n", s_addressing_modes[op->address_mode]);
				break;
			case M68K_OP_FP_SINGLE:
				printf_to_string("\t\toperands[%u].type: FP_SINGLE\n", i);
				printf_to_string("\t\t\toperands[%u].simm: %f\n", i, op->simm);
				break;
			case M68K_OP_FP_DOUBLE:
				printf_to_string("\t\toperands[%u].type: FP_DOUBLE\n", i);
				printf_to_string("\t\t\toperands[%u].dimm: %lf\n", i, op->dimm);
				break;
		}
	}
}
